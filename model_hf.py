...
class DM2FNet(Base):
    def __init__(self, num_features=128, arch='resnext101_32x8d'):
        ...
        self.down1 = nn.Sequential(
            nn.Conv2d(256, num_features, kernel_size=1), nn.SELU(),
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=1, dilation=1), nn.SELU(),
        ...
        self.down3 = nn.Sequential(
            nn.Conv2d(1024, num_features, kernel_size=1), nn.SELU(),
            nn.Conv2d(num_features, num_features, kernel_size=3, padding=4, dilation=4), nn.SELU()
        )
        self.down4 = nn.Sequential(
            nn.Conv2d(2048, num_features, kernel_size=1), nn.SELU()
        )
        self.pyramid_pooling0 = PyramidPooling(64)
        self.pyramid_pooling1 = PyramidPooling(256)
        self.pyramid_pooling2 = PyramidPooling(512)
        self.pyramid_pooling3 = PyramidPooling(1024)
        self.pyramid_pooling4 = PyramidPooling(2048)
    ...
    def forward(self, x0, x0_hd=None):
        ...
        layer0 = self.pyramid_pooling0(layer0)
        layer1 = backbone.layer1(layer0)
        layer1 = self.pyramid_pooling1(layer1)
        layer2 = backbone.layer2(layer1)
        layer2 = self.pyramid_pooling2(layer2)
        layer3 = backbone.layer3(layer2)
        layer3 = self.pyramid_pooling3(layer3)
        layer4 = backbone.layer4(layer3)
        layer4 = self.pyramid_pooling4(layer4)
...
class DM2FNet_woPhy(Base_OHAZE):
    def __init__(self, num_features=64, arch='resnext101_32x8d'):
        ...
        # -----------------------------------
        # self.attention_fuse3 = SelfAttention(num_features)
        # self.attention_fuse2 = SelfAttention(num_features)
        # self.attention_fuse1 = SelfAttention(num_features)
        # self.attention_fuse0 = SelfAttention(num_features)
...
class PyramidPooling(nn.Module):
    def __init__(self, in_channels):
        ...
        self.in_channels = in_channels  # 保存输入通道数为类变量
        self.pools = nn.ModuleList([
            nn.Sequential(
                nn.AdaptiveAvgPool2d(output_size=(s, s)),
                nn.Conv2d(in_channels, in_channels // 4, kernel_size=1),
                nn.ReLU(inplace=True),
                nn.Conv2d(in_channels // 4, in_channels, kernel_size=1)  # 恢复到原始通道数
            ) for s in [1, 2, 4, 8]
        ])
        self.final_conv = nn.Conv2d(in_channels * 5, in_channels, kernel_size=1)  # 调整最终通道数的卷积层

    def forward(self, x):
        original_size = x.size()[2:]  # 获取输入的空间尺寸
        features = [x] + [F.interpolate(pool(x), size=original_size, mode='bilinear', align_corners=True) for pool in self.pools]
        out = torch.cat(features, dim=1)  # 将输出的通道数乘以5 (原始输入 + 4个池化分支)
        out = self.final_conv(out)  # 使用实例变量应用最后的卷积层
        return out
